...
#define A0 (+2.548295920E-01)
#define A1 (-2.844967360E-01)
#define A2 (+1.421413741E+00)
#define A3 (-1.453152027E+00)
#define A4 (+1.061405429E+00)
#define P0 (+3.275911000E-01)
...
#define RATIONAL_TRANSFORM(z) 1.0 / (1.0 + P0*z)

/*  Rational approximation of erf(x) using the exponential function.          */
double Erf_Double_Abramowitz_and_Stegun(double x)
{
    ...
    const double exp_minus_x_sq = tmpl_Double_Exp(-x*x);

    /*  Transform the variable to 1 / (1 + px), p is given above.             */
    const double z = RATIONAL_TRANSFORM(abs_x);

    /*  Evaluate the polynomial using Horner's method.                        */
    const double poly = POLY_EVAL(z);

    /*  The error function can be approximated as follows.                    */
    const double erf_x = 1.0 - poly*exp_minus_x_sq;
...
/*  End of Erf_Double_Abramowitz_and_Stegun.                                  */
